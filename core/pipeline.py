...
class FestivalWorldPipeline:
    ...
    def export_worldpainter_project(self, output_path: Path) -> None:
        ...
        plugin_manifest = {
            ...
            "terrain_export": "heightmap.png",
            "structure_script": "place_structures.we",
            "schematic_dir": str(self.settings.schematic_dir),
            "toolchain": [
                "WorldEdit",
                "WorldPainter",
                "Paper/Fabric-compatible placement",
            ],
        }
        (output_path / "plugin_manifest.json").write_text(json.dumps(plugin_manifest, indent=2))

        plugin_dir = output_path / "plugins" / "festivalworld"
        plugin_dir.mkdir(parents=True, exist_ok=True)
        (plugin_dir / "plugin.yml").write_text(
            """name: FestivalWorld
version: 1.0.0
main: com.festivalworld.Main
api-version: 1.20
commands:
  festivalworld:
    description: Place the exported festival layout
    usage: /festivalworld
"""
        )
        (plugin_dir / "README.md").write_text(
            """# FestivalWorld plugin scaffold

This folder is generated by FestivalWorld Compiler for real-world Minecraft deployment.
Use the exported WorldEdit script and terrain heightmap together with a Paper/Fabric-compatible wrapper.
"""
        )
    ...
    def build_from_geo(
        ...
        try:
            ...
            if satellite_image is not None:
                wb.satellite_image = np.asarray(satellite_image)
